...
This method is computationally inexpensive since the size of the population is small. Furthermore, it helps accurately establish the similarity between any two neural network models. Given two neural network models $S_i$ and $S_j$ if $d(S_j, S_i) = 0$ then $S_1 = S_2$. We say that AMS (Algorithm \ref{algorithm:nn_ea}) has reached nominal convergence if at least $m_c$ pairs of models have $d(S_i, S_j) \leq d_t$, where both $m_c$ and $d_t$ are user defined parameters.
