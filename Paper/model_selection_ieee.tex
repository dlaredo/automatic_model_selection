...
We repeat the experiment with $\alpha = 0.7$, the obtained models are listed next and their fitness, raw size and accuracy are depicted in Table \ref{table:autonn_mnist_3}. The best overall model is $S^+_{0.7} = S^*_4$.
...
Figure ... compares the 15 models. Each model was trained for 100 epochs and evaluated using a cross-validation set. Test and cross-validation sets were the same for all of the models. As mentioned earlier, the sizes of the models obtained when $\alpha = 0.5$ and $\alpha = 0.3$ lie within the same order of magnitude. There are two possible explanations for this behavior; intuitively it can be thought that MNIST exhibits better performance when small networks are used, therefore by trying to improve the performance of the model the algorithm implicitly chooses compact networks, on the other hand the scaling done to the network size (see Equation  \ref{eq:fitness_function_scaled}) is likely contributing to this behavior, since we are using a logaritmic scale to measure the size of the networks a few thousands of weights are unlikely to make a big difference in the fitness of a model. The fact that the accuracy of the obtained models when $\alpha=0.3$ is just slightly better than the results obtained with $\alpha=0.5$, is likely due to the fact that MNIST dataset is usually easy to classify using neural networks. We will further analyze this behavior using other datasets in the next sections.
...
Model & 5-fold Avg. Score & Test Avg. Score & Network size\\
\hline
$S^+_{0.3}$ & 14.82 & 15.34 & 123217\\
$S^+_{0.5}$ & 14.96 & 15.82 & 316369\\
$S^+_{0.7}$ & 15.07 & 16.12 & 46073\\
\hline
\end{tabular}
\end{center}
...
Figure \ref{fig:alpha_cmapss} shows ....
