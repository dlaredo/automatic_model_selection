...
class Configuration():
    ...
    def __init__(self, architecture_type, problem_type, input_shape, output_shape, pop_size, tournament_size,
                 max_similar, size_scaler=1, epochs=1, cross_val=0.2, more_layers_prob=0.5,
                 max_generations=1, binary_selection=True, mutation_ratio=0.4, similarity_threshold=0.9,
                 verbose_individuals=False, show_model=False, verbose_training=0):
...
def evaluate_individual(individual, configuration, data_handler, tModel_scaler, ind_index, unroll,
                        learningRate_scheduler=None):
    ...
    tModel = fetch_to_keras.create_tunable_model(individual.stringModel, individual.problem_type,
                                                 configuration.input_shape, data_handler, ind_index)
    ...
    individual.compute_fitness(epochs=configuration.epochs, cross_validation_ratio=configuration.cross_val,
                               size_scaler=configuration.size_scaler, verbose=configuration.verbose_training,
                               unroll=unroll, learningRate_scheduler=learningRate_scheduler)
    individual.individual_label = ind_index


def evaluate_population(population, configuration, data_handler, tModel_scaler, best_model, worst_model, unroll,
                        learningRate_scheduler=None):
    ...
    for i in range(len(population)):
        ...
        print("verbose individual")
        print(configuration.verbose_individuals)

        if configuration.verbose_individuals == True:
            print("Individual {} score/size/fitness {}/{}/{}".format(i, individual.raw_score, individual.raw_size,
                                                                     individual.fitness))
...
def evaluate_population2(population, configuration, data_handler, tModel_scaler, best_model, worst_model, unroll,
                         verbose_data):
    ...
    for individual in population:
        ...
        individual.compute_fitness(epochs=configuration.epochs, cross_validation_ratio=configuration.cross_val,
                                   size_scaler=configuration.size_scaler, verbose_data=verbose_data, unroll=unroll)
...
def partial_run(model_genotype, problem_type, input_shape, data_handler, cross_validation_ratio, run_number, epochs=20):
    ...
    tModel = SequenceTunableModelRegression('ModelMNIST_SN_' + str(run_number), model, lib_type='keras',
...
def run_experiment(configuration, data_handler, experiment_number, unroll=False, learningRate_scheduler=None,
                   tModel_scaler=None):
    ...
    best_model = nn_evolutionary.Individual(configuration.pop_size * 2, configuration.problem_type, [], [],
                                            fitness=10 ** 8)  # Big score for the first comparisson
    worst_model = nn_evolutionary.Individual(configuration.pop_size * 2 + 1, configuration.problem_type, [], [],
                                             fitness=0)  # Small score for the first comparisson
    worst_index = 0

    # Log the information of this experiment
    logging.info("Starting model optimization: Problem type {}, Architecture type {}".format(configuration.problem_type,
    ...
    logging.info(
        "Input shape: {}, Output shape: {}, cross_val ratio: {}, Generations: {}, Population size: {}, Tournament size: {}, Binary selection: {}, Mutation ratio: {}, Size scaler: {}".format(
            configuration.input_shape, configuration.output_shape, configuration.cross_val,
            configuration.max_generations, configuration.pop_size,
            configuration.tournament_size, configuration.binary_selection, configuration.mutation_ratio,
            configuration.size_scaler))

    population = nn_evolutionary.initial_population(configuration.pop_size, configuration.problem_type,
                                                    configuration.architecture_type,
